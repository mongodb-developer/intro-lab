...
Open the `BookService` class and check the `bookById` method:
...
1. $match: filters the book by _id (just like a SQL WHERE).

2. $lookup: performs a join with the issueDetails collection to get related records.
...
3. $addFields: computes the available field by subtracting issued books from the total inventory.
4. $project: removes the temporary details field before returning the final result.

Just like in Node.js, this approach works but it’s not efficient — it simulates a relational-style query in MongoDB.
